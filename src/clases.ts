...
interface Factura extends Producto{
    precioTotal: number;
}

function CodigoProducto (newUID:string){
    newUID = Math.random().toString(36).slice(2);
    return newUID;
}

function CatalogoProductos (codigoProd:string,nombreProducto: string,precioProducto: number,cantidad:number,canXProd: number){
    ...
    let list: [string,string,number,number,number];
